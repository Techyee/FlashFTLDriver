...
int main(int argc,char* argv[]){
	...
	bench_add(VECTOREDRSET,0		 , RANGE*4/16 - 1, RANGE/4);
	bench_add(VECTOREDRSET,0		 , RANGE*4/16 - 1, RANGE/4);
	bench_add(VECTOREDRSET,0		 , RANGE*4/16 - 1, RANGE/4 * 4);
	bench_add(VECTOREDRSET,0		 , RANGE*4/16 - 1, RANGE * 2);
	printf("range: %lu!\n",RANGE);

	//allocate new thread for interface_main.
	pthread_t pth[5];
	int tid1, tid2, tid3, tid4, tid5, status;
	tinfo = (task_info*)malloc(sizeof(task_info)*5);
	...
	//Best-effort task. period = 0, gc_threshold = -1.
	tinfo[4].bench_idx = 4;
	tinfo[4].num_op = 1;
	tinfo[4].period = 500;
	tinfo[4].gc_threshold = 128;
	tinfo[4].chip_num = 4;
